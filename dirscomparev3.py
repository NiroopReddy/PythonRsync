...
def get_files(directory):
	proc = subprocess.Popen('rsync -av ' +  directory, 
		...
		shell=True)
	output = proc.stdout.read().decode("utf-8").splitlines()[2:-4]

	files = {}	#Empty dictionary to store files
	for line in output:
		fields = line.split()
...
def get_changed_files(directory1, directory2, isFast=False):
	...
	output = proc.stdout.read().decode("utf-8").splitlines()[:-1]
...
if __name__ == '__main__':
	...
	files_changed = get_changed_files_in_both(unique, get_changed_files(local_dir, remote_dir))	#Get files changed in both directories
	#print(files_changed)
